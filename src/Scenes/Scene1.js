class Scene1 extends Phaser.Scene {
    ...
    create(){
        ...
        this.enemyBullets = [];
        for(let i = 0; i < 10; i++){
            this.enemy = new bunnyEnemy(this, bunnyPos + (100*i), 300, "bunnyEnemy1", null, this.bunnyHealth);
    ...
    update(){
        ...
        if(this.enemyBullets && this.enemyBullets.length > 0) {
            this.enemyBullets.forEach(flyBullet => {
                ...
                if(this.collides(flyBullet, my.sprite.player)) {
                    this.playerHealth -= 1;
                    flyBullet.y = 1200;
                    if(this.playerHealthIcon[this.playerHealth] != undefined){
        ...
        if(this.enemyList.length == 0){
            this.scene.start("Scene2", this.myScore);
        }
    }
